document.addEventListener('DOMContentLoaded', function() {
	...
	searchBlocks.forEach(block => {
		...
		input.addEventListener('focus', function() {
			if (this.value.trim() === '') {
				const defaultUrl = getDefaultIcon();
				if (defaultUrl && matchContainer) {
					matchContainer.innerHTML = `<img src="${defaultUrl}" alt="">`;
					matchContainer.classList.add('is-visible');
				}
			}
		...
		input.addEventListener('blur', function() {
			if (this.value.trim() === '' && matchContainer) {
				matchContainer.classList.remove('is-visible');
			}
		});
	});
